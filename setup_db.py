"""
setup_db.py - Auto-initializes the SQLite database for Streamlit Cloud deployment.
Called automatically from app.py on first run if marketing.db doesn't exist.
"""
import sqlite3
import os
...
def init_db():
    """Create and seed the database if it doesn't already exist."""
    ...
    cursor.executemany(
        """INSERT OR IGNORE INTO content_master
    ...
    cursor.executemany(
        """INSERT OR IGNORE INTO content_production
           (content_id,copywriter,writer_status,designer,design_status,video_editor,editing_status,revision_count)
           VALUES (?,?,?,?,?,?,?,?)""",
        [
    ...
    cursor.executemany(
        """INSERT OR IGNORE INTO performance_tracker
           (content_id,reach,impressions,likes,comments,shares,saves,link_clicks,
            leads_generated,revenue_generated,engagement_rate,content_score)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
        [
    ...
    cursor.executemany(
        "INSERT OR IGNORE INTO approvals (content_id,approval_status,requested_by,feedback) VALUES (?,?,?,?)",
        [
    ...
    cursor.executemany(
        """INSERT OR IGNORE INTO content_requests
           (client_id,request_title,description,requested_by,status)
           VALUES (?,?,?,?,?)""",
        [
